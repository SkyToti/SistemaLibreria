...
-- Este script arregla la vulnerabilidad "Function Search Path Mutable" en todas las funciones del sistema.
-- La solución es establecer explícitamente el 'search_path' a 'public' para evitar que usuarios malintencionados
-- puedan inyectar objetos (tablas/funciones) creando esquemas con nombres similares.
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public -- FIX: Establecer search_path seguro
AS $$
BEGIN
  RETURN QUERY
  SELECT 
...
-- 5. Arreglar log_inventory_change
...
DECLARE
  ...
  v_book_title TEXT;
BEGIN
  -- Verificar stock primero (Validación atómica)
