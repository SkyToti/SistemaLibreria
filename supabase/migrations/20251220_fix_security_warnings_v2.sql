-- FIX SECURITY WARNINGS (Versión Corregida)
-- Solución al error 42P13: "cannot change return type of existing function"
-- PostgreSQL no permite cambiar el tipo de retorno con CREATE OR REPLACE si es diferente al anterior.
-- La solución es borrar (DROP) las funciones primero y luego recrearlas con la configuración segura.

-- 1. Arreglar get_dashboard_stats
...
-- 2. Arreglar is_admin (No requiere DROP usualmente si solo cambia el cuerpo, pero por seguridad lo hacemos)
...
-- DROP FUNCTION IF EXISTS public.log_inventory_change() CASCADE; -- Cuidado con CASCADE, mejor REPLACE directo
-- Las funciones trigger suelen ser más flexibles con REPLACE, pero si falla, usaremos DROP
